...
REAL penalty(REAL r, REAL eps, REAL A[3], REAL B[3], REAL  C[3], REAL  D[3], REAL  E[3], REAL  F[3], REAL  P[3], REAL  Q[3], unsigned *totit);
...
REAL serial_pen_test(unsigned n, REAL r, REAL eps, REAL* a[3], REAL* b[3], REAL*  c[3], REAL*  d[3], REAL*  e[3], REAL*  f[3], REAL*  p[3], REAL*  q[3], REAL distance[], REAL fails[], REAL *failrate, unsigned totit[])
{
    REAL *distcmp = new REAL[n];
    REAL A[3],B[3], C[3], D[3], E[3], F[3], P1[3], P2[3], Q1[3], Q2[3], PP[3], QQ[3], P3[3], Q3[3], P[3], Q[3];
    ...
    for(unsigned i=0;i<n;i++)
    {
        ...
        distance[i] = penalty(r, eps, A, B, C, D, E, F, P1, Q1, &totit[i]);
        ...
        *failrate = (REAL(REAL(counter)/REAL(n))*100.0);
    }
      
    return sqrt(DOT(PP,PP) + DOT(QQ,QQ))/sqrt(DOT(Q3,Q3)+DOT(P3,P3));
}

REAL penalty(REAL r, REAL eps, REAL A[3], REAL B[3], REAL  C[3], REAL  D[3], REAL  E[3], REAL  F[3], REAL  P[3], REAL  Q[3], unsigned *totit)
{
    REAL BA[3], CA[3], ED[3], FD[3], hessian[16];
    ...
    hessian[15] = 2.*DOT(FD,FD);//10 FLOPS + 5 FLOPS
    ...
    REAL x[4]; // = {0.33, 0.33, 0.33, 0.33}; // initial guess
    ...
    REAL error, dx[4], a[16], SUBXY[3], b[4], mx[6], dh[8], tmp1, tmp2, tmp3, tmp4, tmp5, tmp6;
    ...
    *totit = i;
    ...
    SUBXY[0] = (P[0]) - (Q[0]);
    SUBXY[1] = (P[1]) - (Q[1]);
    SUBXY[2] = (P[2]) - (Q[2]);//21 FOPS
    return sqrt(DOT(SUBXY,SUBXY));//5 FLOPS
}

void serial_bf(unsigned n, REAL* a[3], REAL* b[3], REAL*  c[3], REAL*  d[3], REAL*  e[3], REAL*  f[3], REAL*  p[3], REAL*  q[3], REAL distance[])
{
    REAL A[3],B[3], C[3], D[3], E[3], F[3], P[3], Q[3];
